...
export default function AddArtist() {
  ...
  useEffect(() => {
   ...
   function onSubmit () {
    ...
    const newAlbum = {name:albumName, youtubelink: link, artistId: id[0].id + 1}
    console.log(newAlbum);
   axios.post('/artists',  newArtist )
   .catch((data) => setProblem(data.response.data.message));
   console.log(problem)
   axios.post('/albums',  newAlbum )
   .catch((data) => setProblem(data.response.data.message));
  ...
  >
    ...
    <Form >
      ...
      <Button  disabled={loading}
              className="w-100"
              type="submit"
              variant="danger"onClick={onSubmit}>
        Submit
      </Button>
    </Form>
    </Card.Body>
